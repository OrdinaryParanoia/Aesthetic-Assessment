...
optimizer_oflstm = optim.SGD([
        {'params': rnn.parameters(), 'lr': 0.01}], momentum=0.9
        )
...
for i in range(EPOCH2):
    ...
    for step, image_DATA1 in enumerate(image_loader1):
        temp_input = model(image_DATA1['image'].to(device))
        
        input_oflstm = temp_input.view(-1,1,1).float().to(device)
        labels_oflstm = image_DATA1['annotations'].to(device)
...
#        print(loss_oflstm)
    ...
    for step, image_DATA2 in enumerate(image_loader2):
        temp_input = model(image_DATA2['image'].to(device))
        
        input_oflstm = temp_input.view(-1,1,1).float().to(device)
        labels_oflstm = image_DATA2['annotations'].to(device)
        optimizer_oflstm.zero_grad()
        output_oflstm, (hn, cn) = rnn(input_oflstm)
        if output_oflstm.shape[0] != 16:
            continue
